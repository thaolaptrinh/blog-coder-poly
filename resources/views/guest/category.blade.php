<x-guest-layout class="index feed-view search-view">
    ...
    <x-slot name="metaTags">
        ...
        <title> {{ Str::ucfirst($category->name) }} - {{ blog()->site_name }} </title>
        <meta name="description" content="{{ Str::ucfirst(words($category->description, 120)) }}">
        ...
        <!-- Open Graph -->
        <meta property="og:title" content="{{ Str::ucfirst($category->name) }} - {{ blog()->site_name }}">
        <meta property="og:description" content="{{ Str::ucfirst(words($category->description, 120)) }}">
        ...
        <meta name="twitter:title" property="og:title" itemprop="name"
            content="{{ Str::ucfirst(words($category->description, 120)) }}">
        <meta name="twitter:description" property="og:description"
            content="{{ Str::ucfirst(words($category->description, 120)) }}">
        <meta name="twitter:label1" content="Posts" />
        <meta name="twitter:data1" content="{{ $category->posts_count }}" />
                ...
                <div class='main section' id='main' name='Main Recent Posts'>
                    <div class='widget Blog' data-version='2' id='Blog1'>
                        <div class="queryMessage">
                            <span class="query-info query-success">{{ $category->name }}</span>
                        </div>
